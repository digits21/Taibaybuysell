...
u.username name,u.phone tel,u.date time,u.city,
...
FROM users u
INNER JOIN mobiles m ON u.id=m.user_id
INNER JOIN furnitures f ON u.id=f.user_id
INNER JOIN fashion fas ON u.id=fas.user_id
INNER JOIN electronics e ON u.id=e.user_id
INNER JOIN cars c ON u.id=c.user_id
INNER JOIN bikes b ON u.id=b.user_id
WHERE m.title LIKE ?
ORDER BY u.date DESC LIMIT 9');
    /*$req->bindParam('mbrand',$search);
     $req->bindParam('fbrand',$search);
     $req->bindParam('fasbrand',$search);
     $req->bindParam('ebrand',$search);
     $req->bindParam('cbrand',$search);
     $req->bindParam('bbrand',$search);*/
       ...
       echo "NO ".$search." found";
        ...
        echo "".$res['mob_title']."";
				            ...
				            echo'<span class="cityname">City:'.$row['mob_cit'].'</span>';
                                 ...
                                 echo'<span class="date">Phone:'  .$row['tel'].'</span>';             
									echo'</section>';
									echo'<div class="clearfix"></div>';
									echo'</li> </a>';
                                          echo'</ul>';  }
                          ...
                          echo'<div role="tabpanel" class="tab-pane fade in active" id="profile" aria-labelledby="home-tab">';
						   echo'<div>';
								...
								echo'<div class="clearfix"></div>';
                                     ...
                                     // include("connect.php");
                                        $i=1;
//$city_2=isset($_GET['city'])?$_GET['city']:false;
