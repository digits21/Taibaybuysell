 <?php
                                      include("connect.php");
                                        $i=1;
$req=$pdo->query('SELECT users.username,users.email,users.city,users.phone,users.link,users.city,users.date,mobiles.title,mobiles.description,mobiles.image,mobiles.price FROM users, mobiles WHERE users.id=mobiles.user_id ORDER BY mobiles.price LIMIT 9');
